...
CREATE TABLE IF NOT EXISTS `retailer-final-project-gcp.silver_dataset.customers`
(
    customer_id INT64,
    name STRING,
    email STRING,
...
MERGE INTO  `retailer-final-project-gcp.silver_dataset.customers` target
USING 
  (SELECT DISTINCT
    *, 
    CASE 
    ...
    True as is_active
  FROM `retailer-final-project-gcp.bronze_dataset.customers`) source
...
CREATE TABLE IF NOT EXISTS `retailer-final-project-gcp.silver_dataset.orders`
(
    order_id INT64,
    customer_id INT64,
    order_date STRING,
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `retailer-final-project-gcp.silver_dataset.orders` target
USING 
  (SELECT DISTINCT
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `retailer-final-project-gcp.bronze_dataset.orders`) source
...
CREATE TABLE IF NOT EXISTS `retailer-final-project-gcp.silver_dataset.order_items`
(
    order_item_id INT64,
    order_id INT64,
    product_id INT64,
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `retailer-final-project-gcp.silver_dataset.order_items` target
USING 
  (SELECT DISTINCT
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `retailer-final-project-gcp.bronze_dataset.order_items`) source
...
CREATE TABLE IF NOT EXISTS `retailer-final-project-gcp.silver_dataset.categories`
(
    category_id INT64,
    name STRING,
    updated_at STRING,
...
TRUNCATE TABLE `retailer-final-project-gcp.silver_dataset.categories`;

--Step 3: Insert New or Updated Records
INSERT INTO `retailer-final-project-gcp.silver_dataset.categories`
SELECT 
  *,
  CASE 
...
FROM `retailer-final-project-gcp.bronze_dataset.categories`;

--Step 1: Create the products Table in the Silver Layer
CREATE TABLE IF NOT EXISTS `retailer-final-project-gcp.silver_dataset.products`
(
  product_id INT64,
  name STRING,
  category_id INT64,
...
--Step 2: Truncate table
TRUNCATE TABLE `retailer-final-project-gcp.silver_dataset.products`;

--Step 3: Insert New or Updated Records
INSERT INTO `retailer-final-project-gcp.silver_dataset.products`
SELECT 
  *,
  CASE 
    WHEN category_id IS NULL OR name IS NULL THEN TRUE
...
FROM `retailer-final-project-gcp.bronze_dataset.products`;
-------------------------------------------------------------------------------------------------------------
--Step 1: Create the product_supplier Table in the Silver Layer
CREATE TABLE IF NOT EXISTS `retailer-final-project-gcp.silver_dataset.product_suppliers`
(
    supplier_id INT64,
    product_id INT64,
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `retailer-final-project-gcp.silver_dataset.product_suppliers` target
USING 
  (SELECT 
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `retailer-final-project-gcp.bronze_dataset.product_suppliers`) source
...
CREATE TABLE IF NOT EXISTS `retailer-final-project-gcp.silver_dataset.suppliers`
(
  supplier_id INT64,
...
--Step 2: Truncate table
TRUNCATE TABLE `retailer-final-project-gcp.silver_dataset.suppliers`;

--Step 3: Insert New or Updated Records
INSERT INTO `retailer-final-project-gcp.silver_dataset.suppliers`
SELECT 
  *,
  CASE 
...
FROM `retailer-final-project-gcp.bronze_dataset.suppliers`;
-------------------------------------------------------------------------------------------------------------

--Step 1: Create the customer_reviews Table in the Silver Layer
CREATE TABLE IF NOT EXISTS `retailer-final-project-gcp.silver_dataset.customer_reviews`
(
    id STRING,
    customer_id INT64,
    product_id INT64,
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `retailer-final-project-gcp.silver_dataset.customer_reviews` target
USING 
  (SELECT 
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `retailer-final-project-gcp.bronze_dataset.customer_reviews`) source
